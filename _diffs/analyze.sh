...
echo "SHA,Files,Added,Removed" > _diffs/stats.csv

for sha in $SHAS; do
  stats=$(git diff --numstat $sha..origin/main -- $PATHS 2>/dev/null | awk '{files++; added+=$1; removed+=$2} END {print files","added","removed}')
  if [ -z "$stats" ]; then
    stats="0,0,0"
  fi
  echo "$sha,$stats" >> _diffs/stats.csv
done
