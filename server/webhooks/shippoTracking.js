...
function verifyShippoSignature(req, webhookSecret) {
  ...
  const isValid = crypto.timingSafeEqual(Buffer.from(shippoSignature), Buffer.from(signature));
  ...
  try {
    ...
    for (const transaction of transactions) {
      ...
      if (
        ...
        console.log(
          `✅ Found transaction ${transaction.id} with tracking number ${trackingNumber}`
        );
        return transaction;
      }
    }
...
async function handleTrackingWebhook(req, res, opts = {}) {
  ...
  if (!skipSignature) {
    ...
    if (webhookSecret) {
      ...
      console.log(
        `${testPrefix}⚠️ SHIPPO_WEBHOOK_SECRET not set - skipping signature verification (test mode)`
      );
    }
  } else {
  ...
  try {
    ...
    console.log(
      `✅ Shippo mode check passed: event.mode=${event?.mode || 'none'}, expected=${expectedMode ||
        'any'}`
    );
    ...
    if (!upperStatus || (!isDelivery && !isFirstScan)) {
      console.log(
        `ℹ️ Status '${trackingStatus}' is not DELIVERED or first-scan status - ignoring webhook`
      );
      return res.status(200).json({ message: `Status ${trackingStatus} ignored` });
    }

    console.log(
      `✅ Status is ${upperStatus} - processing ${isDelivery ? 'delivery' : 'first scan'} webhook`
    );
    ...
    console.log(
      `🔍 Tracking type: ${
        isReturnTracking ? 'RETURN' : 'OUTBOUND'
      } (metadata.direction=${metadata.direction || 'none'})`
    );
    ...
    if (isReturnTracking && isFirstScan) {
      ...
      console.log(
        `[TRACKINGLINK] Using short public link for return: ${publicTrackingUrl} (carrier: ${returnCarrier ||
          'unknown'})`
      );
      ...
      try {
        await sendSMS(lenderPhone, message, {
          ...
          meta: {
            listingId:
              transaction.attributes.listing?.id?.uuid || transaction.attributes.listing?.id,
    ...
    if (!isReturnTracking) {
      ...
      if (isFirstScan) {
        ...
        if (pdFirstScanSent || cacheValid) {
          console.log(
            `ℹ️ [STEP-4] First scan SMS already sent - skipping (idempotent via ${
              pdFirstScanSent ? 'protectedData' : 'cache'
            })`
          );
      ...
      if (isDelivery) {
        ...
        if (!trackingNum) {
          ...
          return res
            .status(400)
            .json({ error: 'No tracking number found for first scan notification' });
        }

        // Generate public carrier tracking URL (shorter than Shippo URLs)
        const publicTrackingUrl = getPublicTrackingUrl(carrier, trackingNum);
        console.log(
          `[TRACKINGLINK] Using short public link: ${publicTrackingUrl} (carrier: ${carrier ||
            'unknown'})`
        );
        ...
        console.log(
          `[STEP-4] Sending borrower SMS for tracking ${trackingNumber}, txId=${transaction.id}`
        );
        console.log(
          `[STEP-4] Message length: ${message.length} chars, shortLink: ${shortTrackingUrl}`
        );
        protectedDataUpdate = {
          ...protectedData,
          lastTrackingStatus: {
      ...
      try {
        ...
        if (isFirstScan) {
          console.log(
            `✅ [STEP-4] Borrower SMS sent for tracking ${trackingNumber}, txId=${transaction.id}`
          );
        } else {
        ...
        try {
          ...
          console.error(
            `❌ Failed to update transaction protectedData for ${smsType}:`,
            updateError.message
          );
